...
export default function Login() {
  const { user, isLoading, login, loginWithGoogle, config } = useAuth();
  ...
  const onSubmit = async (vals: FormVals) => {
    ...
    try {
      ...
      toast({ title: "Sign-in failed", description: e.message?.includes("401") ? "Invalid email or password" : e.message, variant: "destructive" });
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <Card className="w-full max-w-md p-8 rounded-xl bg-card border-card-border">
        <div className="flex flex-col items-center gap-3 mb-6">
          ...
          <div className="text-center">
            <h1 className="text-xl font-semibold tracking-tight" data-testid="text-login-title">Welcome to WorkTrack</h1>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4" data-testid="form-login">
          ...
          <div>
            ...
            <Input id="password" type="password" autoComplete="current-password" {...form.register("password")} data-testid="input-password" />
        ...
        {googleEnabled ? (
          <div className="flex justify-center" ref={googleDivRef} data-testid="container-google-signin" />
        ) : (
        ...
        <p className="text-sm text-center text-muted-foreground mt-6">
          ...
          <Link href="/signup" className="text-primary hover:underline" data-testid="link-signup">Sign up</Link>
        </p>
      </Card>
    </div>
  );
